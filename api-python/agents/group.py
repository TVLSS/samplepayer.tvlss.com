...
def _get_group(_i, s):
    return {k: v for k, v in s.group.items() if k not in ("invoices", "renewal", "pending", "enrollmentByMonth")}


def _enrollment(_i, s):
    return {"eligibleEmployees": s.group["eligibleEmployees"], "enrolledSubscribers": s.group["enrolledSubscribers"], "participationRate": round(s.group["enrolledSubscribers"] / s.group["eligibleEmployees"] * 1000) / 10, "totalCoveredLives": s.group["totalCoveredLives"], "byPlan": s.group["plans"], "enrollmentByMonth": s.group["enrollmentByMonth"], "pending": s.group["pending"]}


def _renewal(_i, s):
    return {"renewalDate": s.group["renewalDate"], **s.group["renewal"]}


def _invoices(i, s):
    return [inv for inv in s.group["invoices"] if not i.get("status") or i["status"] == "Any" or inv["status"] == i["status"]]


def _invoice_detail(i, s):
    inv = next((x for x in s.group["invoices"] if x["invoiceId"].upper() == str(i.get("invoice_id")).upper().strip()), None)
    if not inv:
...
def _add_employee(i, s):
    ...
    plan = next((p["name"] for p in s.group["plans"] if p["planId"] == i.get("plan_id")), None)
    r = {"changeId": f"ENR-{1000 + len(s.changes) + 1}", "employee": i.get("employee_name"), "plan": plan, "coverageTier": i.get("coverage_tier"), "hireDate": i.get("hire_date"), "effectiveDate": eff, "summary": f"{i.get('employee_name')} enrolled effective {eff}", "note": "Demo only: nothing was actually enrolled."}
    s.changes.append(r)
    s.group["enrolledSubscribers"] += 1
    return r
...
def _terminate(i, s):
    ...
    r = {"changeId": f"TRM-{500 + len(s.changes) + 1}", "employee": i.get("employee_name"), "lastDay": d, "coverageEnds": end, "cobraNoticeMailed": "Within 14 days", "summary": f"{i.get('employee_name')} coverage ends {end}", "note": "Demo only."}
    s.changes.append(r)
    return r
