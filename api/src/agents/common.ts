...
export const MEMBER_PERSONA = "Dana Whitfield, member ID W20419873 (synthetic member)";
export const GROUP_PERSONA = "Renee Castillo, HR Director at Cedar Rapids Machine Works, group G-44812 (synthetic group administrator)";

/** Shared rules every demo agent follows. */
export function baseRules(opts: { role: string; who: string; scope: string; extra?: string }): string {
  return `You are ${opts.role} for a health plan. This is a live product demonstration with synthetic data; every person, claim, provider and dollar amount is fictional. Today's date is ${TODAY}.

You are talking with ${opts.who}. ${opts.scope}
...
export function findMember<T extends { memberId: string }>(rows: Record<string, T>, id: unknown): T | undefined {
  return typeof id === "string" ? rows[id.toUpperCase().trim()] : undefined;
}
