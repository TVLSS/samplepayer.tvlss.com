...
const state = JSON.parse(JSON.stringify(group)) as typeof group;
...
const tools: Tool[] = [
  {
    ...
    run: () => { const { invoices, renewal, pending, enrollmentByMonth, ...g } = state; return g; },
  },
  {
    ...
    input_schema: { type: "object", properties: {}, additionalProperties: false },
    run: () => ({ eligibleEmployees: state.eligibleEmployees, enrolledSubscribers: state.enrolledSubscribers, participationRate: Math.round((state.enrolledSubscribers / state.eligibleEmployees) * 1000) / 10, totalCoveredLives: state.totalCoveredLives, byPlan: state.plans, enrollmentByMonth: state.enrollmentByMonth, pending: state.pending }),
  },
  {
    ...
    input_schema: { type: "object", properties: {}, additionalProperties: false },
    run: () => ({ renewalDate: state.renewalDate, ...state.renewal }),
  },
  {
    ...
    run: (i) => state.invoices.filter((inv) => !i.status || i.status === "Any" || inv.status === i.status),
  },
  {
  ...
  },
  {
    ...
    run: (i) => { const hire = new Date(String(i.hire_date) + "T00:00:00Z"); if (isNaN(hire.getTime())) return { error: "hire_date must be YYYY-MM-DD" }; const plus30 = new Date(hire.getTime() + 30 * 86400000); const eff = new Date(Date.UTC(plus30.getUTCFullYear(), plus30.getUTCMonth() + 1, 1)).toISOString().slice(0, 10); const r = { changeId: `ENR-${String(1000 + changes.length + 1)}`, employee: i.employee_name, plan: state.plans.find((p) => p.planId === i.plan_id)?.name, coverageTier: i.coverage_tier, hireDate: i.hire_date, effectiveDate: eff, summary: `${i.employee_name} enrolled effective ${eff}`, note: "Demo only: nothing was actually enrolled." }; changes.push(r); state.enrolledSubscribers += 1; return r; },
  },
  {
    ...
    run: (i) => { const d = String(i.last_day); const end = new Date(Date.UTC(Number(d.slice(0, 4)), Number(d.slice(5, 7)), 0)).toISOString().slice(0, 10); const r = { changeId: `TRM-${String(500 + changes.length + 1)}`, employee: i.employee_name, lastDay: d, coverageEnds: end, cobraNoticeMailed: "Within 14 days", summary: `${i.employee_name} coverage ends ${end}`, note: "Demo only." }; changes.push(r); return r; },
  },
  {
    name: "list_changes", system: "GROUP_ADMIN", kind: "read",
    description: "Lists enrollment changes made during this conversation.",
    input_schema: { type: "object", properties: {}, additionalProperties: false },
    run: () => changes,
  },
];
