...
import { familyIds, plans, providers } from "../data.js";
...
const tools: Tool[] = [
  {
    ...
    run: (i, s) => { const m = s.members[who(i.member_id) as keyof typeof s.members]; return { ...m, plan: plans[m.planId as keyof typeof plans].name }; },
  },
  {
    ...
    run: (_i, s) => familyIds.map((id) => { const m = s.members[id]; return { memberId: id, name: m.name, relationship: m.relationship, dob: m.dob, status: m.status, coverageStart: m.coverageStart, coverageEnd: m.coverageEnd }; }),
  },
  {
  ...
  },
  {
    ...
    run: (i, s) => { const p = providers.find((p) => p.npi === String(i.npi)); if (!p) return { error: `No provider with NPI ${i.npi}` }; if (!["Family Medicine", "Pediatrics", "Internal Medicine"].includes(p.specialty)) return { error: `${p.name} is ${p.specialty}; a PCP must be family medicine, internal medicine or pediatrics` }; const m = s.members[who(i.member_id) as keyof typeof s.members]; m.pcp = { npi: p.npi, name: p.name, practice: p.practice }; const r = { requestId: `PCP-${m.memberId}`, member: m.name, newPcp: m.pcp, effective: "2026-10-01", summary: `PCP changed to ${p.name}`, note: "Demo only." }; s.requests.push(r); return r; },
  },
  {
